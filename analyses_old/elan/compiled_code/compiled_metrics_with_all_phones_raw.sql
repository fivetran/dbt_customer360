...
with zendesk as (
    ...
    from `private-internal`.`zz_dbt_jamie_zendesk_source`.`stg_zendesk__user`
    left join `private-internal`.`zz_dbt_jamie_zendesk_source`.`stg_zendesk__organization`
...
, customer_name) as customer_organization_name,
        coalesce(
...
, shipping_name) as shipping_organization_name,
        coalesce(replace( 
...
, ')', ''), shipping_name) as shipping_full_name

    from `private-internal`.`zz_dbt_jamie_stripe`.`stripe__customer_overview`
...
marketo as (
    ...
    select *
    from `private-internal`.`zz_dbt_jamie_marketo`.`marketo__leads` where email is not null -- since we're using email to match records here
),
